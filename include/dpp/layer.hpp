...
class Layer {
	...
	protected:
		...
		void block_softsign(Eigen::MatrixXf &H, Eigen::MatrixXf &dH, const size_t &index) {
		...
		void matrix_softsign(Eigen::MatrixXf &H, Eigen::MatrixXf &dH) {
			dH = H.array().abs();
		...
		void tensor_softsign(Eigen::Tensor<float, 3> &H, Eigen::Tensor<float, 3> &dH) {
			Eigen::Tensor<float, 3> denom = 1.0 + H.abs(); 
			H = H / denom;	
			dH = 1.0 / denom.square();
		}
		...
		void conv_tanh(const Eigen::Tensor<float, 3> &X,
			...
			dO(j) = ((Eigen::Tensor<float, 0>) (1.0 - product.square()).sum())(0) * mean_forward_;
        }
		...
		void conv_softsign(const Eigen::Tensor<float, 3> &X,
                           const Eigen::Tensor<float, 4> &K,
                           const Eigen::Tensor<float, 1> &B,
                           Eigen::Tensor<float, 1> &O,                     
                           Eigen::Tensor<float, 1> &dO,                    
			...
			Eigen::Tensor<float, 3> product = 1.0 + (X.slice(X_off, X_ext) * K.slice(K_off, K_ext).reshape(X_ext)).abs();
            
			O(j) = ((Eigen::Tensor<float, 0>) (X.slice(X_off, X_ext) * K.slice(K_off, K_ext).reshape(X_ext) / product).sum())(0) * mean_forward_ + B(start); 
		...
		void set_activation(std::function <void (Eigen::MatrixXf &, Eigen::MatrixXf &)> &act_fun, const std::string &activation) {
			act_fun = std::bind(&Layer::matrix_normal, this, std::placeholders::_1, std::placeholders::_2);

			if (activation == "sigmoid") {
				act_fun = std::bind(&Layer::matrix_sigmoid, this, std::placeholders::_1, std::placeholders::_2);
			} else if (activation == "relu") {
				act_fun = std::bind(&Layer::matrix_relu, this, std::placeholders::_1, std::placeholders::_2);
			} else if (activation == "tanh") {
				act_fun = std::bind(&Layer::matrix_tanh, this, std::placeholders::_1, std::placeholders::_2);
			} else if (activation == "softmax"){
				act_fun = std::bind(&Layer::matrix_softmax, this, std::placeholders::_1, std::placeholders::_2);
			} else {
				act_fun = std::bind(&Layer::matrix_softsign, this, std::placeholders::_1, std::placeholders::_2);
			}
		}

		void set_activation(std::function <void (Eigen::Tensor<float, 3> &, Eigen::Tensor<float, 3> &)> &act_fun, const std::string &activation) {
			act_fun = std::bind(&Layer::tensor_normal, this, std::placeholders::_1, std::placeholders::_2);
			
			if (activation == "sigmoid") {
				act_fun = std::bind(&Layer::tensor_sigmoid, this, std::placeholders::_1, std::placeholders::_2);
			} else if (activation == "relu") {
				act_fun = std::bind(&Layer::tensor_relu, this, std::placeholders::_1, std::placeholders::_2);
			} else if (activation == "tanh") {
				act_fun = std::bind(&Layer::tensor_tanh, this, std::placeholders::_1, std::placeholders::_2);
			} else if (activation == "softmax"){
				act_fun = std::bind(&Layer::tensor_softmax, this, std::placeholders::_1, std::placeholders::_2);
			} else {
				act_fun = std::bind(&Layer::tensor_softsign, this, std::placeholders::_1, std::placeholders::_2);
			}
		}
		
		void set_activation(std::function <void (Eigen::MatrixXf &, Eigen::MatrixXf &, const size_t &)> &act_fun, const std::string &activation) {
			act_fun = std::bind(&Layer::block_normal, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);

			if (activation == "sigmoid") {
				act_fun = std::bind(&Layer::block_sigmoid, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
			} else if (activation == "softmax") {
                act_fun = std::bind(&Layer::block_softmax, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
            } else if (activation == "relu") {
                act_fun = std::bind(&Layer::block_relu, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
            } else if (activation == "softsign") {
                act_fun = std::bind(&Layer::block_softsign, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
            } else {
				act_fun = std::bind(&Layer::block_tanh, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
			}
		}

		void set_activation(std::function <void (const Eigen::Tensor<float, 3> &,
			...
			act_fun = std::bind(&Layer::conv_normal, this, std::placeholders::_1,
								std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5, std::placeholders::_6,
								std::placeholders::_7, std::placeholders::_8, std::placeholders::_9, std::placeholders::_10, std::placeholders::_11);

			if (activation == "relu") {
				act_fun = std::bind(&Layer::conv_relu, this, std::placeholders::_1,
									  std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5, std::placeholders::_6,
                                      std::placeholders::_7, std::placeholders::_8, std::placeholders::_9, std::placeholders::_10, std::placeholders::_11);
			} else if (activation == "tanh") {
				act_fun = std::bind(&Layer::conv_tanh, this, std::placeholders::_1,
                                      std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5, std::placeholders::_6,
                                      std::placeholders::_7, std::placeholders::_8, std::placeholders::_9, std::placeholders::_10, std::placeholders::_11);
			} else if (activation == "sigmoid") {
			    act_fun = std::bind(&Layer::conv_sigmoid, this, std::placeholders::_1,
                                      std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5, std::placeholders::_6,
									  std::placeholders::_7, std::placeholders::_8, std::placeholders::_9, std::placeholders::_10, std::placeholders::_11);
			} else if (activation == "softmax") {
				act_fun = std::bind(&Layer::conv_softmax, this, std::placeholders::_1,
                                      std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5, std::placeholders::_6,
                                      std::placeholders::_7, std::placeholders::_8, std::placeholders::_9, std::placeholders::_10, std::placeholders::_11);
			} else {
				act_fun = std::bind(&Layer::conv_softsign, this, std::placeholders::_1,
                                      std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5, std::placeholders::_6,
                                      std::placeholders::_7, std::placeholders::_8, std::placeholders::_9, std::placeholders::_10, std::placeholders::_11);
			}
		}	
