#ifndef IMG_PARSER_HPP
#define IMG_PARSER_HPP
...
void load_mnist(const char* image_filename, const char* label_filename, std::vector<Eigen::Tensor<float,3>> &img, std::vector<Eigen::Tensor<float,3>> &labels){
    // Open files
	...
	labels.resize(num_items); img.resize(num_items);
