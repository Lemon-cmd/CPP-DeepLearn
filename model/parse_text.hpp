...
class parse_text
{
	...
	public:
		...
		const std::vector<Eigen::MatrixXf>& get_X() { return X; }
		const std::vector<Eigen::MatrixXf>& get_Y() { return Y; }
		const auto& vsize() { return id; }
		const std::vector<Eigen::MatrixXf>& get_Xtrain() { return X_train; }
		const std::vector<Eigen::MatrixXf>& get_Ytrain() { return Y_train; }
		const std::vector<Eigen::MatrixXf>& get_Xtest() { return X_test; }
		const std::vector<Eigen::MatrixXf>& get_Ytest() { return Y_test; }
};
